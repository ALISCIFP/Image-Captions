...
if from_checkpoint:
    ...
    if torch.cuda.is_available():
        if bert_model:
            ...
            encoder_checkpoint = torch.load('./checkpoints_baseline/encoder_bert')
            decoder_checkpoint = torch.load('./checkpoints_baseline/decoder_bert')
        elif glove_model:
            print('Pre-Trained GloVe Model')
            encoder_checkpoint = torch.load('./checkpoints_baseline/encoder_glove')
            decoder_checkpoint = torch.load('./checkpoints_baseline/decoder_glove')
        else:
            print('Pre-Trained Baseline Model')
            encoder_checkpoint = torch.load('./checkpoints_baseline/encoder_baseline')
            decoder_checkpoint = torch.load('./checkpoints_baseline/decoder_baseline')
    else:
        if bert_model:
            print('Pre-Trained BERT Model')
            encoder_checkpoint = torch.load('./checkpoints_baseline/encoder_bert', map_location='cpu')
            decoder_checkpoint = torch.load('./checkpoints_baseline/decoder_bert', map_location='cpu')
        elif glove_model:
            print('Pre-Trained GloVe Model')
            encoder_checkpoint = torch.load('./checkpoints_baseline/encoder_glove', map_location='cpu')
            decoder_checkpoint = torch.load('./checkpoints_baseline/decoder_glove', map_location='cpu')
        else:
            print('Pre-Trained Baseline Model')
            encoder_checkpoint = torch.load('./checkpoints_baseline/encoder_baseline', map_location='cpu')
            decoder_checkpoint = torch.load('./checkpoints_baseline/decoder_baseline', map_location='cpu')
...
def train():
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        for i, (imgs, caps, caplens) in enumerate(tqdm(train_loader)):
            ...
            if i%5000==0 and i!=0:
                ...
                torch.save({
                    ...
                    'loss': loss,
                    }, './checkpoints_baseline/decoder_mid')

                torch.save({
                    'epoch': epoch,
                    'model_state_dict': encoder.state_dict(),
                    'loss': loss,
                    }, './checkpoints_baseline/encode_mid')
        ...
        torch.save({
            ...
            'optimizer_state_dict': decoder_optimizer.state_dict(),
            'loss': loss,
            }, './checkpoints_baseline/decoder_epoch'+str(epoch+1))

        torch.save({
            'epoch': epoch,
            'model_state_dict': encoder.state_dict(),
            'loss': loss,
            }, './checkpoints_baseline/encoder_epoch'+str(epoch+1))
